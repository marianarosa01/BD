...
FROM (GESTSTOCK.FORNECEDOR LEFT OUTER JOIN GESTSTOCK.ENCOMENDA ON nif=fornecedor) 
...
-- c)   
SELECT numEnc, avg(unidades) as numUnidades
FROM GESTSTOCK.ITEM
GROUP BY numEnc;

-- d) 
SELECT GESTSTOCK.FORNECEDOR.nome, GESTSTOCK.PRODUTO.nome,sum(GESTSTOCK.ITEM.unidades) as quantidade
FROM (GESTSTOCK.PRODUTO JOIN 
								(GESTSTOCK.ITEM JOIN
														(GESTSTOCK.FORNECEDOR JOIN GESTSTOCK.ENCOMENDA ON nif = fornecedor)
														ON numEnc = numero)
								ON Codigo = CodProd)
GROUP BY GESTSTOCK.FORNECEDOR.nome, GESTSTOCK.PRODUTO.nome